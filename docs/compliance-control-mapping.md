...
| CIS-style node/control-plane checks | kube-bench CronJob (`deploy/kube-bench`, `kubeBench.enabled`) | Use managed-cluster baselines for honest posture; kind results differ from DOKS/AKS. Document waived or accepted findings per environment. |
